...
from azureml.pipeline.wrapper.dsl.component import componentExecutor, StringParameter, InputDirectory, OutputDirectory
...
class TransformerNet(torch.nn.component):
    def __init__(self):
...
class ConvLayer(torch.nn.component):
...
class ResidualBlock(torch.nn.component):
    """ResidualBlock
...
class UpsampleConvLayer(torch.nn.component):
    """UpsampleConvLayer
...
#for parallel run component, the input and output need to be defined in dsl.component(parallel_inputs = ***) like below
@dsl.component(
    job_type='parallel',
    description='use pretrained pythorch model to do image style transfer',
    name='Style Transfer Parallel',
    parallel_inputs=[InputDirectory(name='Content Dir', description="directory of input images")]
)
def style_transform_parallel(
    model_dir: InputDirectory(description="saved torch model to be used for stylizing the image."),
    output_path: OutputDirectory(description="directory holding the output images"),
...
if __name__ == '__main__':
    componentExecutor(style_transform_parallel).execute(sys.argv)